...
export function Dialog({ open, onClose, title, children }: DialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4 backdrop-blur-sm safe-x safe-top safe-bottom" role="presentation" onClick={onClose}>
      <div className="glass max-h-[min(88svh,46rem)] w-full max-w-3xl overflow-hidden rounded-[1.8rem] shadow-soft" role="dialog" aria-modal="true" aria-label={title} onClick={(event) => event.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          <div>
            <p className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Workspace Dialog</p>
