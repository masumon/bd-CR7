...
const defaultProductionApi = normalizeApiUrl(process.env.PROXY_DEFAULT_PYTHON_API || "");
  ...
  NextResponse.json(
    {
      success: false,
      data: null,
      error: `Proxy loop detected: resolved API target points to current origin (${requestOrigin}). Set PYTHON_API_URL to the actual Python backend URL. Target was: ${targetUrl}`,
    },
    { status: 508 },
  );
